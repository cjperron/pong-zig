...
pub fn init(allocator: std.mem.Allocator, options: struct {
    ...
    var titulo = try Widget.initUnderlinedText(allocator, .{
        .text = "PONG",
        .layout_info = .{ .Anchored = .{ .anchor = .Top, .offset_y = 200 } },
        .font_size = 120,
    });
    ...
    const jugar_btn_ctx = struct {
        pub fn call(self: *const @This()) void {
            _ = self;
            // TODO: chequear que el nombre del jugador ya exista en algun otro lado y lanzar el juego con ese nombre.

            const app_state = AppState.getInstanceMut();
            app_state.requested_scene = .GameplaySetup;
        }
    }{};
    ...
    const button_group_widget = Widget.initWidgetGroup(.{
        .widgets = main_menu_buttons,
        .layout_info = .{ .Anchored = .{ .anchor = .Center, .offset_x = 0, .offset_y = 150 } },
        .spacing = 20,
    });
